...
# ---------------------------------------------------------------------------
DEFAULT_PORT=7456
DEFAULT_IMAGE="docker.io/townsendwu/open-design:latest"
...
generate_api_token() {
  ...
  error "Cannot generate OPEN_DESIGN_API_TOKEN: openssl and od are both unavailable."
  exit 1
}
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --api-token) shift; OPT_API_TOKEN="$1" ;;
    --api-token=*) OPT_API_TOKEN="${1#--api-token=}" ;;
    ...
    --help|-h)
      ...
      echo "  --api-token <token>     API bearer token (default: generated)"
...
if [ "$NON_INTERACTIVE" = "0" ]; then
  ...
  prompt_text "API token" "$API_TOKEN"
