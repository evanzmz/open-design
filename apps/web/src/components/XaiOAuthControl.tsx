...
async function fetchStatus(): Promise<XaiAuthStatus | null> {
  try {
    const r = await fetch(apiUrl('/api/xai/auth/status'), { credentials: 'same-origin' });
    if (!r.ok) return null;
...
> {
  try {
    const r = await fetch(apiUrl('/api/xai/oauth/start'), {
      method: 'POST',
...
async function disconnectOAuth(): Promise<boolean> {
  try {
    const r = await fetch(apiUrl('/api/xai/oauth/disconnect'), {
      method: 'POST',
      credentials: 'same-origin',
    });
    return r.ok;
...
async function cancelInFlightOAuth(): Promise<void> {
  ...
  try {
    await fetch(apiUrl('/api/xai/oauth/cancel'), {
      method: 'POST',
      credentials: 'same-origin',
    });
  } catch {
...
async function completeOAuthManual(
  ...
  try {
    const r = await fetch(apiUrl('/api/xai/oauth/complete'), {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      credentials: 'same-origin',
